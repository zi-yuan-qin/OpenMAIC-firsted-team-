...
- **前置任务**：`P1-001`（需要新的 ProviderId 注册机制）
- **负责人**：队长
...
- **前置任务**：`P1-001` 和 `P1-002`
- **负责人**：队员1
- **产出位置**：`__tests__/`

**检验标准：**

- [ ] 类型注册表测试：ProviderId 注册/注销/查询
- [ ] Zod schema 验证测试：合法输入通过、非法输入拒绝
- [ ] 策略模式测试：各提供商 thinking 参数生成正确
- [ ] 适配器测试：MiniMax/Google 适配器输出正确格式
- [ ] 健康检查测试：模拟提供商超时/失败返回正确状态
- [ ] 故障转移测试：主提供商失败时自动切换到备用
- [ ] 成本追踪测试：token 计数和费用计算准确
...
## Phase 2 · 多智能体重构 (Week 3-5)

> 目标：编排引擎拆解 + 提示词系统替换 + 智能体注册表重构
...
- **前置任务**：`P2-001`（编排引擎就绪）+ `P2-002`（角色定义就绪）
- **负责人**：队员2
...
> 为编排层、提示词系统、智能体注册表编写测试

- **前置任务**：`P2-001`, `P2-002`, `P2-003`
- **负责人**：队员1
- **产出位置**：`__tests__/`

**检验标准：**

- [ ] 导演决策测试：单智能体模式规则引擎输出正确
- [ ] 导演决策测试：多智能体模式 LLM 路由返回有效 JSON
- [ ] 图结构测试：节点连接顺序正确，循环逻辑无死锁
- [ ] 状态管理测试：消息裁剪保留关键信息
...
__tests__/
  orchestration/
    graph-definition.test.ts
    director-node.test.ts
    agent-node.test.ts
...
## Phase 3 · 生成管线重构 (Week 6-7)

> 目标：两阶段管线拆分 + JSON 修复增强
...
- [ ] Widget 推断逻辑重写（§3.1：场景生成管线逻辑中的启发式替换）
...
- **前置任务**：`P1-002`（提供商层就绪）+ `P2-001`（编排引擎就绪）
- **负责人**：队长
...
- [ ] 速率限制测试：超限请求被正确拒绝
- [ ] API 版本测试：v1 路由与旧路由共存
- [ ] OpenAPI 文档测试：生成的 spec 覆盖所有端点
- [ ] 性能监控测试：指标采集和上报正确
- [ ] 错误追踪测试：异常被正确记录
...
- **前置任务**：`P1-003`, `P2-004`, `P3-003`, `P4-003`, `P5-003`（所有单元测试完成）
- **负责人**：队长
- **产出位置**：`__tests__/e2e/`
...
├── P1-001 类型系统标准化 ──────────── 队员1
│   ├── P1-002 提供商层优化 ────────── 队长
│   │   └── P1-003 Phase 1 测试 ────── 队员1
│   │
Phase 2 (Week 3-5)
├── P2-001 编排引擎重构 ────────────── 队长
│   ├── P2-002 提示词系统重构 ──────── 队员2
│   │   ├── P2-003 智能体注册表重构 ── 队员2
│   │   │   └── P2-004 Phase 2 测试 ── 队员1
│   │
Phase 3 (Week 6-7)
├── P3-001 两阶段管线拆分 ──────────── 队员1
│   ├── P3-002 JSON 修复增强 ───────── 队员3
│   │   └── P3-003 Phase 3 测试 ────── 队员1 + 队员3
│
Phase 4 (Week 8-9)
├── P4-001 状态管理优化 ────────────── 队员3
│   ├── P4-002 组件架构优化 ────────── 队员3
│   │   └── P4-003 Phase 4 测试 ────── 队员2 + 队员3
│
Phase 5 (Week 10)
├── P5-001 API 层重构 ──────────────── 队长
│   ├── P5-002 测试与监控系统 ──────── 队员2
│   │   └── P5-003 Phase 5 测试 ────── 队员2
│
Phase 6 (Week 11-12)
├── P6-001 端到端集成测试 ──────────── 队长
│   ├── P6-002 性能调优 ────────────── 队长
...
| **队长** | P5-001 | API 层重构（错误处理 + 速率限制 + 版本管理 + OpenAPI） | Week 10 |
| **队长** | P6-001 | 端到端集成测试（20 项全功能测试） | Week 11-12 |
| **队长** | P6-002 | 性能调优（10 项优化策略） | Week 11-12 |
| **队长** | P6-003 | 文档完善（8 份文档） | Week 11-12 |
| **队员1** | P1-001 | 类型系统标准化（Zod + 运行时注册 + JSDoc） | Week 1-2 |
| **队员1** | P1-003 | Phase 1 单元测试（7 个测试文件） | Week 1-2 |
| **队员1** | P2-004 | Phase 2 单元测试（11 个测试文件） | Week 3-5 |
| **队员1** | P3-001 | 两阶段生成管线拆分（内容生成器 + 缓存 + 质量评分） | Week 6-7 |
| **队员1** | P3-003 | Phase 3 单元测试（11 个测试文件） | Week 6-7 |
| **队员2** | P2-002 | 提示词系统重构（21 个模板重写 + 组合引擎 + A/B + 多语言） | Week 3-5 |
| **队员2** | P2-003 | 智能体注册表重构（模板系统 + 工厂 + 组合规则） | Week 3-5 |
| **队员2** | P4-003 | Phase 4 前端测试（11 个测试文件） | Week 8-9 |
| **队员2** | P5-002 | 测试与监控系统（集成测试 + 性能监控 + 错误追踪 + Docker） | Week 10 |
| **队员2** | P5-003 | Phase 5 基础设施测试（9 个测试文件） | Week 10 |
| **队员2** | P6-003 | 文档完善（协助队长） | Week 11-12 |
| **队员3** | P3-002 | JSON 修复与解析增强（可插拔管道 + Zod 验证 + 遥测） | Week 6-7 |
| **队员3** | P3-003 | Phase 3 单元测试（协助队员1） | Week 6-7 |
| **队员3** | P4-001 | 状态管理优化（5 个子 store + 迁移 + 预设 + 导入导出） | Week 8-9 |
| **队员3** | P4-002 | 组件架构优化（agent-bar 拆分 + Canvas 框架 + 主题 + 响应式） | Week 8-9 |
| **队员3** | P4-003 | Phase 4 前端测试（协助队员2） | Week 8-9 |
...
| 内容 | 位置 | 替换方案 | 对应任务 |
|------|------|---------|---------|
| 21 个提示词模板 | `lib/prompts/templates/*` | 全部重写为新提示词 | P2-002 |
| 默认智能体人设 | `registry/store.ts` 6 个默认 agent | 替换为通用英文版本 | P2-003 |
| 角色行为准则 | `prompt-builder.ts` roleGuidelines | 重写为通用教学角色定义 | P2-002 |
| 导演路由规则 | `director-prompt.ts` | 重写为通用对话路由 | P2-002 |
| 动作描述系统 | `tool-schemas.ts` | 重写为通用工具描述 | P2-002 |
| 场景生成管线逻辑 | `scene-generator.ts` 启发式规则 | 重写 widget 推断逻辑 | P3-001 |
| 中文 fallback 语音文本 | `scene-generator.ts` | 改为英文/多语言 | P3-001 |
| 格式化工具硬编码文本 | `prompt-formatters.ts` | 替换为可配置模板 | P3-001 |
