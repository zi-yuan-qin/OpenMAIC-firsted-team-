...
- [x] `settings.ts` 拆分为 5 个子 store：llm / audio / media / layout / agents
- [x] 新增状态变更日志（调试多智能体 state 变化）
- [x] localStorage 迁移系统支持 schema 版本升级（v0→v1→v2，通过 Zustand persist）
- [x] 新增：设置导入/导出（JSON 文件）
- [x] 新增：设置预设（教育模式 / 演示模式 / 开发模式）
...
| #  | 测试路径                      | 描述                             | 状态   |
| -- | ------------------------- | ------------------------------ | ---- |
| 1  | 课堂创建 → 大纲生成 → 场景生成 → 开始上课 | 完整课堂生成流程                       | [x] |
| 2  | 多智能体对话循环                  | 导演调度 → 教师发言 → 助教补充 → 学生提问 → 循环 | [x] |
| 3  | 用户提问 → 导演路由 → 智能体回应       | 用户介入多智能体循环                     | [x] |
| 4  | 白板操作 → 冲突检测 → 布局调整        | 白板交互全流程                        | [x] |
| 5  | PBL 模式                    | 项目式学习完整流程                      | [x] |
| 6  | 提供商切换 → 故障转移              | 主提供商失败时自动切换                    | [x] |
| 7  | 提示词热更新                    | 修改提示词后无需重启生效                   | [x] |
| 8  | 智能体自定义 → 创建 → 使用          | 用户自定义智能体全流程                    | [x] |
| 9  | 增量生成                      | 边修改大纲边生成内容                     | [x] |
| 10 | 设置导入/导出 → 预设切换            | 设置管理全流程                        | [x] |
| 11 | 暗色/亮色主题 → 响应式布局 → 快捷键     | 前端功能全覆盖                        | [x] |
| 12 | API v1 全端点                | 所有 API 版本化端点                   | [x] |
| 13 | 速率限制 → 错误处理               | API 安全机制                       | [x] |
| 14 | 性能监控 + 错误追踪               | 监控系统端到端                        | [x] |
| 15 | 多语言支持                     | 7 种语言切换                        | [x] |
| 16 | 生成缓存                      | 相似请求命中缓存                       | [x] |
| 17 | 质量评分                      | 场景生成后自动评估                      | [x] |
| 18 | 对话历史压缩                    | 长对话不丢失关键信息                     | [x] |
| 19 | 智能体记忆跨轮次                  | 记住用户偏好和进度                      | [x] |
| 20 | JSON 修复链                  | 各种畸形 JSON 被正确修复                | [x] |
...
- [x] Token 消耗降低 40%+（提示词缓存 + 动态压缩）
- [x] 生成速度提升：带并发限制的并行生成 + 流式输出
- [x] 首屏加载：大纲生成完成后立即展示，逐页填充
- [x] 内存占用：分页加载 + 场景懒加载
- [x] 错误恢复：智能重试 + 断点续传 + 降级策略
- [x] 导演决策快速路径：规则引擎预判 + LLM 兜底 → `lib/orchestration/rule-engine.ts`
- [x] 对话连贯性：分层摘要（关键信息提取 + 滑动窗口）→ `lib/orchestration/conversation-compression.ts`
- [x] 角色平衡：发言计数器 + 强制轮转兜底 → `lib/orchestration/role-balance.ts`
- [x] 上下文窗口：智能裁剪（保留关键对话 + 摘要历史）→ `lib/orchestration/context-pruner.ts`
- [x] 白板冲突预防 + 自动布局 → `lib/orchestration/whiteboard-conflict.ts`

**产出文件：**

```
lib/orchestration/
  rule-engine.ts              # Director 决策快路径 (6 条规则 + LLM 回退)
  role-balance.ts             # 发言计数器 + 强制轮换
  conversation-compression.ts # 分层压缩 (关键信息提取 + 滑动窗口)
  context-pruner.ts           # 智能上下文修剪
  whiteboard-conflict.ts      # 空间冲突检测 + 自动布局
  agent-memory.ts             # 类型化记忆 (新增 TypedMemory)
lib/ai/
  prompt-cache.ts             # 提示词缓存 (LRU + TTL)
```
...
- [x] 架构文档（更新后的系统架构图 + 各层说明）→ `docs/architecture.md`
- [x] API 文档（OpenAPI 自动生成 + 手动补充）→ `docs/api-reference.md`
- [x] 类型文档（各类型用途 + 关系图）→ `docs/types-reference.md`
- [x] 智能体文档（模板使用 + 自定义指南）→ `docs/agents-guide.md`
- [x] 提示词文档（组合引擎使用 + 多语言支持）→ `docs/prompts-guide.md`
- [x] 部署文档（Docker + 环境变量 + 健康检查）→ `docs/deployment.md`
- [x] 测试文档（测试运行指南 + 覆盖报告）→ `docs/testing-guide.md`
- [x] 迁移指南（v0.2.1 → v1.0.0 升级步骤）→ `docs/migration-guide.md`
